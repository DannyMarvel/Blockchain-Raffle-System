...
contract CreateSubscription is Script {
    ...
    //The we create a subsscription here
    ...
    function run() public {
        CreateSubscriptionUsing();
    }
}

contract FundSubcription is Script, CodeConstants {
    uint256 public constant FUND_AMOUNT = 3 ether;//Also equals 3 link
    function fundSubscriptionUsingConfig() public {
        HelperConfig helperConfig = new HelperConfig();
        address vrfCoordinator = helperConfig.getConfig().vrfCoordinator;
        uint256 subscriptionId = helperConfig.getConfig().vrfCoordinator;
        address linkToken = helperConfig.getConfig().link;
        address account = helperConfig.getConfig().account;
        FundSubcription(vrfCoordinator, subscriptionId, linkToken, account);
    }

function fundSubscription(address vrfCoordinator, uint256 subscriptionId, address linkToken,
